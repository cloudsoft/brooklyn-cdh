...
public class WhirrClouderaManagerImpl extends WhirrClusterImpl implements WhirrClouderaManager {

    public static final Logger log = LoggerFactory.getLogger(WhirrClouderaManagerImpl.class);

    static {
        RendererHints.register(WhirrClouderaManager.CLOUDERA_MANAGER_URL, new RendererHints.NamedActionWithUrl("Open"));
    }

    public WhirrClouderaManagerImpl(Entity owner) {
        this([:], owner);
    }
    public WhirrClouderaManagerImpl(Map flags = [:], Entity owner = null) {
        super(flags, owner)
...
//            FirewallManager.authorizeIngress(computeServiceContext, instances, clusterSpec, cidrs, ports);
    ...
    protected void customizeClusterSpecConfiguration(JcloudsLocation location, PropertiesConfiguration config) {
        ...
        if (location.getRegion())
            templateSpec += ",locationId="+location.getRegion();
